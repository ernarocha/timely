  ...
  useEffect(() => {
    ...
    const onKeyDown = (event) => {
      ...
      if (event.key === 'Tab' && panelRef.current) {
        const items = [...panelRef.current.querySelectorAll('button, input, textarea, select, [tabindex]:not([tabindex="-1"])')].filter((item) => !item.disabled)
    ...
    requestAnimationFrame(() => panelRef.current?.querySelector('input, textarea, select, button')?.focus())
    return () => {
