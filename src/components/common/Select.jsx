...
export default function Select({
  ...
  return (
    <div ref={wrapperRef} className="relative min-w-0">
      ...
      >
        <span
          className={`flex min-w-0 items-center gap-2.5 truncate ${selected ? '' : 'text-muted/70 dark:text-white/45'}`}
        >
          {selected?.color && (
            <span
        ...
        <ChevronDown
          size={18}
          className={`shrink-0 text-muted transition-transform duration-200 dark:text-white/50 ${open ? 'rotate-180' : ''}`}
        />
      </button>
      ...
      {open && (
        <div
          ...
          className="absolute left-0 right-0 top-[calc(100%+8px)] z-50 origin-top animate-pop overflow-hidden rounded-2xl border border-primary-container/45 bg-white p-1.5 shadow-lift dark:border-white/10 dark:bg-[#2b2b39]"
        >
          {options.map((option) => (
            <button
              key={option.value}
            ...
            >
              ...
              {option.value === value && (
                <Check size={16} className="shrink-0 text-secondary dark:text-primary-container" />
              )}
            </button>
          ))}
        </div>
