...
import {
  ...
  MIN_ENTRY_HOURS,
  isValidEntryHours,
} from '../../utils/timeEntryValidation'
...
export default function TimeEntryModal({ open, onClose, onSave }) {
  ...
  /* eslint-disable react-hooks/set-state-in-effect -- Opening the controlled modal starts a fresh entry. */
  useEffect(() => {
  ...
  const submit = (event) => {
    ...
    if (!isValidEntryHours(hours))
      return setError(`Hours must be between ${MIN_ENTRY_HOURS} and ${MAX_ENTRY_HOURS}.`)
    onSave({
  ...
  return (
    <Modal
      ...
      description="Log the work while it’s fresh."
    >
      <form className="space-y-5" onSubmit={submit}>
        <div className="space-y-3">
          ...
          <Textarea
            label="Description"
            meta={`${form.description.length}/200`}
            name="description"
        ...
        <div className="flex flex-col gap-3 pt-1 sm:flex-row sm:items-end sm:justify-between">
          <p className="whitespace-nowrap font-mono text-[10px] text-muted dark:text-white/50">
            {format(currentTime, 'MMM d, yyyy · h:mm a')}
          </p>
          <div className="flex gap-3">
            <Button variant="soft" className="flex-1 sm:flex-none" onClick={onClose}>
              Cancel
            </Button>
            <Button variant="lime" className="flex-1 sm:flex-none" type="submit">
